...
- [ ] `app/components/charts/CashFlowChart.tsx` — 예측 차트 컴포넌트 (월 클릭 이벤트 지원)
- [ ] `app/components/charts/NetWorthChart.tsx` — 순자산 추이 차트
- [ ] `app/(protected)/dashboard/ForecastDrilldown.tsx` — 예측 월별 드릴다운 컴포넌트
  - 특정 월 클릭 시 해당 월의 예측 내역을 4유형별로 분리 표시:
    - 고정 반복: 항목별 (급여 +350만, 월세 -70만, 넷플릭스 -1.7만 등)
    - 변동 반복: EMA 예측값 (식비 -24만, 교통비 -6만 등)
    - 예정 비반복: 해당 월 등록 항목 (자동차보험 연납 -42만)
    - 불확실 발생: 준비금 적립액 (부조금 -10만, 의료비 -5만)
  - 각 유형별 소계 + 월 합계
- [ ] `lib/forecast/projection.ts` 확장 — 월별 4유형 상세 breakdown 반환

**verify**: 대시보드에서 이번 달 요약 + 6개월 예측 그래프 + 순자산 추이 확인 + 월 클릭 시 4유형 드릴다운
...
- [ ] `app/(protected)/onboarding/page.tsx` — 온보딩 플로우
  ...
  - Step 3: **초기 잔액 입력** — 각 결제 수단의 현재 잔액/미결제 금액 입력 (가계부 시작 시점의 자산/부채 설정)
    - 복식부기: `Dr. 자산계정 / Cr. 기초순자산(equity)` 개시 분개 자동 생성
    - 부채(신용카드 미결제 등): `Dr. 기초순자산(equity) / Cr. 부채계정`
  - Step 4: 기본 카테고리 확인/커스터마이징
  - Step 5: AI API 키 입력 (선택)
  - 완료 → 기본 계정과목 시드 + 결제수단별 자산/부채 계정 + 개시 분개 + user_settings 생성
...
## 미구현/부분구현 잔여 목록 (QA 검토 2026-03-27)

### ❌ 미구현

| # | 항목 | 설명 | 우선순위 |
|---|------|------|---------|
| 1 | 거래 수정 | 거래 상세에서 edit 기능 없음 (삭제만 가능). "과거 데이터 자유 수정" 요구사항 미충족. | P1 |
| 2 | 자산 등록 유도 | 거래 입력 시 기준액 초과 알림 + 자산 등록 제안 연동 없음 | P2 |
| 3 | 준비금 차감 | 지출 발생 시 준비금에서 차감하는 UI/로직 없음 | P2 |
| 4 | 카테고리 AI 추천 | 새 카테고리 필요 시 AI 추천 기능 없음 | P3 |

### ⚠️ 부분 구현

| # | 항목 | 설명 | 우선순위 |
|---|------|------|---------|
| 1 | 이미지 업로드 UI | 백엔드(API+파서) 완성, 프론트엔드 업로드/파싱결과 확인 UI 없음 | P1 |
| 2 | 선불비용 배분 연동 | accrual.ts 계산함수 있지만 recurring 엔진에서 is_prepaid 미사용 | P1 |
| 3 | 거래 목록 필터 | 기간별/카테고리별/금액범위 필터 누락 (유형+텍스트만) | P2 |
| 4 | top-down 분배 UX | 총액 먼저 입력 후 분배하는 UX 없음 (bottom-up만) | P3 |
| 5 | 카테고리 계층 UI | parent_id 데이터모델만 있고 UI에서 미활용 | P3 |
| 6 | 카테고리/계정 수정 | 삭제만 가능, 이름 변경 등 edit 불가 | P3 |
| 7 | 기준액 분기별 제안 | 수동 조정만 가능, 시스템 자동 제안 없음 | P3 |
