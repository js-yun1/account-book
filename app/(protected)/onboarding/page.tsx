...
export default function OnboardingPage() {
  ...
  function addPaymentMethod() {
    if (!newPmName) return;
    setPaymentMethods([...paymentMethods, { name: newPmName, method_type: newPmType }]);
    setNewPmName("");
  }
  ...
  async function handleComplete() {
    ...
    // 2. 결제 수단별 자산/부채 계정 생성 + payment_methods 등록
    ...
    // 3. 설정 업데이트
    const threshold = Math.round(monthlyExpense * 0.1); // 월 지출의 10%
    ...
    <div className="max-w-lg mx-auto space-y-6 py-8">
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <button onClick={() => setStep(2)} className="w-full rounded-lg bg-blue-600 px-4 py-3 text-sm font-medium text-white hover:bg-blue-700 transition-colors">
            다음
          </button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            사용하는 결제 수단을 등록하세요. 복식부기에서 돈의 출처를 추적하는 데 필요합니다.
          </p>
          ...
          <div className="space-y-2">
            {paymentMethods.map((pm, i) => (
              <div key={i} className="flex items-center gap-2 rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2">
                ...
                <span className="text-xs text-gray-500">{pm.method_type}</span>
          ...
          <div className="flex gap-2">
            <button onClick={() => setStep(1)} className="flex-1 rounded-lg border border-gray-300 dark:border-gray-700 px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">이전</button>
            <button onClick={() => setStep(3)} className="flex-1 rounded-lg bg-blue-600 px-4 py-3 text-sm font-medium text-white hover:bg-blue-700 transition-colors">다음</button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => setStep(2)} className="flex-1 rounded-lg border border-gray-300 dark:border-gray-700 px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">이전</button>
            <button onClick={() => setStep(4)} className="flex-1 rounded-lg bg-blue-600 px-4 py-3 text-sm font-medium text-white hover:bg-blue-700 transition-colors">다음</button>
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-4 space-y-2 text-sm">
            ...
            <p>결제 수단: {paymentMethods.map((pm) => pm.name).join(", ")}</p>
          ...
          <div className="flex gap-2">
            <button onClick={() => setStep(3)} className="flex-1 rounded-lg border border-gray-300 dark:border-gray-700 px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">이전</button>
            <button onClick={handleComplete} disabled={loading} className="flex-1 rounded-lg bg-blue-600 px-4 py-3 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50 transition-colors">
              {loading ? "설정 중..." : "시작하기"}
            </button>
          </div>
        </div>
